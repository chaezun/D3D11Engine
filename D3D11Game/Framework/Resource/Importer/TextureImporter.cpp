	...
	: context(context)
{
	...
	auto FreeImageErrorHandler = [](FREE_IMAGE_FORMAT fif, const char* message)
	{
		...
		const char* format = (fif != FIF_UNKNOWN) ? FreeImage_GetFormatFromFIF(fif) : "Unkown format";
...
auto TextureImporter::Load(const std::string & path,ITexture * texture) -> const bool
{
	if (!texture)
	{
	...
	{
		LOG_ERROR_F("Unknown or unsupported format.");
		return false;
	}
	...
	texture->SetTransparent(image_transparency);
...
auto TextureImporter::GetBitsFromFIBITMAP(std::vector<std::byte>* data, FIBITMAP * bitmap, const uint & width, const uint & height, const uint & channels) -> const bool
{
...
void TextureImporter::GenerateMipmaps(FIBITMAP * bitmap, ITexture * texture, uint width, uint height, uint channels)
{
	if (!texture)
	{
	...
	while (width > 1 && height > 1)
	{
		...
		std::vector<std::byte>* mip = texture->AddMipLevel();
		mip->reserve(size);
		mip->resize(size);
	}
	...
	for (auto& job : jobs)
	{
		...
		{
			FIBITMAP* bitmapScaled = FreeImage_Rescale(bitmap, job.width, job.height, Rescale::rescale_filter);
			if (!GetBitsFromFIBITMAP(job.data, bitmapScaled, job.width, job.height, job.channels))
				LOG_ERROR_F("Failed to create mip level %dx%d", job.width, job.height);
...
auto TextureImporter::ComputeChannelCount(FIBITMAP * bitmap) -> const uint
{
	if (!bitmap)
	{
...
auto TextureImporter::ComputeBitsPerChannel(FIBITMAP * bitmap) -> const uint
{
	if (!bitmap)
	{
...
auto TextureImporter::ComputeTextureFormat(const uint & bpp, const uint & channels) -> const DXGI_FORMAT
{
	if (channels == 3)
	{
...
auto TextureImporter::IsVisuallyGrayscale(FIBITMAP * bitmap) -> const bool
{
	if (!bitmap)
	{
...
auto TextureImporter::ApplyBitmapCorrections(FIBITMAP * bitmap) -> struct FIBITMAP *
{
	if (!bitmap)
	{
...
auto TextureImporter::ConvertTo32Bits(FIBITMAP * bitmap) -> struct FIBITMAP *
{
	if (!bitmap)
	{
	...
	if (!bitmap)
	{
		LOG_ERROR_F("ImageImporter::ConvertTo32Bits : Failed (%d bpp, %d channels).", FreeImage_GetBPP(previousBitmap), ComputeChannelCount(previousBitmap));
		return nullptr;
	}
...
auto TextureImporter::Rescale(FIBITMAP * bitmap, const uint & width, const uint & height) -> struct FIBITMAP *
{
	...
	FIBITMAP* previousBitmap = bitmap;
	bitmap = FreeImage_Rescale(previousBitmap, width, height, Rescale::rescale_filter);
	if (!bitmap)
	{
...
auto TextureImporter::SwapRedBlue32_Wrapper(FIBITMAP * bitmap) -> const bool
{
