...
void Widget_Inspector::ShowTransform(std::shared_ptr<class Transform>& transform) const
{
	...
	if (Inspector_Data::ComponentBegin("Transform", IconType::Component_Transform, transform, true, false))
	{
		...
		const auto show_vector3 = [&show_float](const char* label, Vector3& value)
		{
			...
			ImGui::TextUnformatted(label);
			ImGui::Indent(15.0f);
			show_float("X", &value.x);
			show_float("Y", &value.y);
			show_float("Z", &value.z);
			ImGui::EndGroup();
		};

		show_vector3("Position", position); ImGui::SameLine();
		show_vector3("Rotation", rotation); ImGui::SameLine();
...
void Widget_Inspector::ShowTerrain(std::shared_ptr<class Terrain>& terrain) const
{
	...
	{
		if (ImGui::Button("PaintTerrain"))
			terrain->SetTerrainMode(TerrainMode::PaintTerrain);
		ImGui::SameLine();
		if (ImGui::Button("PaintTree"))
			terrain->SetTerrainMode(TerrainMode::PaintTree);
		ImGui::SameLine();
		if (ImGui::Button("PaintDetail"))
			terrain->SetTerrainMode(TerrainMode::PaintDetail);
		ImGui::SameLine();
		if (ImGui::Button("Settings"))
			terrain->SetTerrainMode(TerrainMode::Settings);

		auto terrain_mode = terrain->GetTerrainMode();

		switch (terrain_mode)
		{
		case TerrainMode::PaintTerrain:
		{
			ImGui::BeginGroup();
			{
				auto paint_mode = static_cast<int>(terrain->GetPaintMode());

				if (ImGui::RadioButton("Color", &paint_mode, 0))
					terrain->SetPaintMdoe(PaintMode::PaintColor);
				ImGui::SameLine();
				if (ImGui::RadioButton("Alpha", &paint_mode, 1))
					terrain->SetPaintMdoe(PaintMode::PaintAlpha);
				ImGui::SameLine();
				if (ImGui::RadioButton("Texture", &paint_mode, 2))
					terrain->SetPaintMdoe(PaintMode::PaintTexture);
				ImGui::SameLine();
				if (ImGui::RadioButton("Raise", &paint_mode, 3))
					terrain->SetPaintMdoe(PaintMode::Raise);
				ImGui::SameLine();
				if (ImGui::RadioButton("Lower", &paint_mode, 4))
					terrain->SetPaintMdoe(PaintMode::Lower);

				if (paint_mode == 0)
				{
					auto terrain_color = terrain->GetTerrainColor();
					ImGui::ColorEdit4("Terrain Color", terrain_color);
				}

				if (paint_mode == 2)
				{
					auto textures = terrain->GetSplatTextures();
					...
					for (const auto& texture : textures)
					{
						ImGui::Image
						(
							texture.second ? texture.second->GetShaderResourceView() : nullptr,
							ImVec2(80.0f, 80.0f),
							ImVec2(0.0f, 0.0f),
							ImVec2(1.0f, 1.0f),
							ImVec4(1.0f, 1.0f, 1.0f, 1.0f),
						...
						if (auto payload = DragDropEvent::ReceiveDragDropPayload(PayloadType::Texture))
						{
							...
							{
								if (const auto resource = Editor_Helper::Get().resource_manager->Load<Texture2D>(std::get<const char*>(payload->data)))
									terrain->AddSplatTexture(resource);
							}
							catch (const std::bad_variant_access& error)
							{
								LOG_ERROR_F("%s", error.what());
		...
		case TerrainMode::PaintTree:
			break;
		case TerrainMode::PaintDetail:
		{
